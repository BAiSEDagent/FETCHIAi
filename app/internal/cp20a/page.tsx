...
function isEnabled(): boolean {
  return process.env.CP20A_PROOF_ENABLED === '1'
}
...
function ProviderCallList({ calls }: { calls: Cp20aProviderCall[] }) {
  if (calls.length === 0) {
    return (
      <p className="text-[13px] leading-relaxed text-text/62">
        No SerpApi or Firecrawl provider calls were made.
      </p>
    )
  }
...
function SourceAdapterCallList({ calls }: { calls: Cp20aSourceAdapterCall[] }) {
  if (calls.length === 0) {
    return (
      <p className="text-[13px] leading-relaxed text-text/62">
        No TDLR source-adapter calls were made.
      </p>
    )
  }
      ...
      {calls.map((call) => (
        ...
        >
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <div className="text-[12px] font-bold uppercase tracking-[0.08em] text-text/45">
                {call.adapter} / {call.county} County
              </div>
              <div className="mt-1 text-[14px] font-bold text-text">
                {call.status.toUpperCase()}
              </div>
            ...
            <div className="text-right text-[12px] font-bold text-text/55">
              {call.candidatesAccepted}/{call.recordsReturned} accepted
            </div>
          </div>
          <p className="mt-2 text-[13px] leading-relaxed text-text/64">
            {call.detail}
...
function Header({ proof }: { proof: Cp20aProofResult }) {
  const statusText =
    ...
    <header className="flex flex-col gap-4 border-b border-text/10 pb-5 md:flex-row md:items-end md:justify-between">
      <div>
        <div className="mb-3 inline-flex items-center gap-2 rounded-md border border-text/10 bg-white px-3 py-1.5 text-[12px] font-bold text-text/65">
          ...
          Internal admin proof - {CP20A_PROOF_ROUTE}
        </div>
        <h1 className="font-outfit text-[38px] font-bold leading-tight text-text md:text-[52px]">
          CP20A TDLR Source-Adapter Proof
        </h1>
        <p className="mt-3 max-w-3xl text-[15px] leading-relaxed text-text/65">
          Live-only DFW Commercial Cleaning source-adapter proof. It renders only
          the runtime result from SerpApi validation, TDLR listing JSON, and
          Firecrawl detail-page hydration.
        </p>
      </div>
...
function BlockedProof({ proof }: { proof: Extract<Cp20aProofResult, { status: 'blocked' }> }) {
  return (
    <>
      <section className="grid gap-5 lg:grid-cols-[1fr_0.9fr]">
        <article className="rounded-lg border border-text/10 bg-white p-5 shadow-[0_14px_36px_rgba(10,10,10,0.07)] md:p-6">
          ...
          <dl>
            ...
            />
            <ProofRow
              label="Source adapter run IDs"
              value={
                proof.liveLineage.sourceAdapterRunIds.length > 0 ? (
                  <div className="space-y-1">
                    {proof.liveLineage.sourceAdapterRunIds.map((runId) => (
            ...
            />
            <ProofRow label="CP20A done" value="No - live accepted opportunity is not rendered." />
          </dl>
        </article>
      ...
      <section className="rounded-lg border border-text/10 bg-white p-5 shadow-[0_14px_36px_rgba(10,10,10,0.07)] md:p-6">
        <div className="mb-5 flex items-center gap-2">
          ...
          <h2 className="font-outfit text-[24px] font-bold text-text">
            TDLR Source-Adapter Calls
          </h2>
        </div>
        <SourceAdapterCallList calls={proof.proof.sourceAdapterCalls} />
      </section>
    </>
  )
}

function ReadyProof({ proof }: { proof: Extract<Cp20aProofResult, { status: 'ready' }> }) {
  return (
    <>
      ...
      <section className="grid gap-5 lg:grid-cols-2">
        <article className="rounded-lg border border-text/10 bg-white p-5 shadow-[0_12px_30px_rgba(10,10,10,0.05)] md:p-6">
          ...
          <dl className="mt-5">
            ...
            />
            <ProofRow label="Source adapter" value={proof.lineage.sourceAdapter} />
            <ProofRow
              label="Source adapter run IDs"
              value={
                <div className="space-y-1">
        ...
        <article className="rounded-lg border border-text/10 bg-white p-5 shadow-[0_12px_30px_rgba(10,10,10,0.05)] md:p-6">
          ...
          <dl className="mt-5">
            ...
            <ProofRow label="Official listing endpoint" value={proof.proof.officialListingEndpoint} />
            <ProofRow label="Provider calls" value={proof.proof.providerCalls.length} />
            <ProofRow label="Source-adapter calls" value={proof.proof.sourceAdapterCalls.length} />
            <ProofRow label="Estimated provider spend" value={money(proof.proof.estimatedProviderSpendUsd)} />
      ...
      <section className="grid gap-5 lg:grid-cols-3">
        <GateList title="TDLR Source Adapter" items={proof.proof.sourceAdapterReasons} />
        <GateList title="Product-Scope Guard" items={proof.proof.productScopeGuardReasons} />
...
export default async function Cp20aInternalProofPage() {
  ...
  const proof = await getCp20aTdlrTabsProof()
