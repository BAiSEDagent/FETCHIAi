        ...
        {messages.map(m => (
          <ChatBubble key={m.id} role={m.role} trailing={m.leads && m.leads.length > 0 ? (
            <div className="flex flex-col gap-3 pt-1">
              {m.leads.map(l => <LeadCard key={l.opportunityId} href={`/app/leads/${l.opportunityId}`} businessName={l.businessName} signalLabel={l.signalLabel} signalType={l.signalType} signalToken={l.signalToken} score={l.score} status={l.status} location={l.location ?? null} ageLabel={l.ageLabel ?? null} variant="chat" />)}
            </div>
