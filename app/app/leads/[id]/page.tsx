  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="px-4 lg:px-7 pb-[calc(env(safe-area-inset-bottom)+96px)] lg:pb-12 space-y-3 lg:space-y-4">
        <section className="rounded-[20px] bg-brand-cream shadow-fetchi-card px-5 py-7 lg:px-8 lg:py-9 text-center">
          <div className="flex items-center justify-center flex-wrap gap-1.5">
            <span
              className="inline-flex items-center rounded-full px-3 py-1.5 text-[11px] font-bold tracking-[0.04em] tabular-nums bg-brand-light text-brand-dark"
              aria-label={`Signal ${signalToken ?? signalTypeLabel}`}
            >
              {signalToken ?? signalTypeLabel.toUpperCase()}
            </span>
            {opp.status === 'new' && (
              <span className="inline-flex items-center rounded-full px-3 py-1.5 text-[11px] font-semibold bg-brand-cream-muted text-brand-near-black/65">
                No claim filed
              </span>
            )}
          </div>

          <div className="font-outfit text-[72px] lg:text-[86px] leading-none font-bold tabular-nums mt-6 text-brand-green">
            {opp.score}
          </div>
          <p className="text-body-lg text-brand-near-black/65 mt-3 px-2">
            {summaryLine}
          </p>

          <h1 className="font-outfit text-h1 lg:text-[32px] text-brand-near-black mt-6 px-2">
            {businessName}
          </h1>
          {locationLine && (
            <div className="text-caption text-brand-near-black/55 mt-1.5">
              {locationLine}
            </div>
          )}
        </section>
        ...
        {opp.whyNow && (
          <SectionCard eyebrow="Why now" tone="highlight">
            <p className="text-body text-brand-near-black/75">
              {opp.whyNow}
            </p>
          </SectionCard>
        )}
        ...
        >
          <div className="space-y-0 -mx-1">
            ...
            {drafts.length > 0 && (
              <EvidenceRow glyph="sparkle" title="Outreach draft ready" meta="Reviewable below" />
            )}
            {evidenceCount === 0 && (
              ...
              value={
                prospect?.website ? (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="text-brand-green hover:underline truncate inline-block max-w-full"
                  >
                    {prospect.website}
                  </a>
                ) : (
            ...
            {contacts.length === 0 ? (
              ...
              <div className="space-y-0">
                {contacts.map(c => (
                  <div
                    key={c.id}
                    className="flex items-center gap-3 py-2.5 border-b border-brand-near-black/8 last:border-0"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-[12px] text-brand-near-black/55 truncate">
                        {c.contactTitle ?? '—'}
                        {c.contactEmail && <span className="text-brand-green"> · {c.contactEmail}</span>}
                      </div>
                    </div>
                  </div>
                ))}
        ...
        <div className="flex flex-col items-center gap-2 pt-1 pb-2">
          ...
          <div className="flex items-center justify-center gap-3 text-[13px] font-semibold text-brand-near-black/55">
            <button type="button" className="min-h-[44px] px-2 hover:text-brand-near-black">Save for later</button>
            <span aria-hidden>·</span>
            <button type="button" className="min-h-[44px] px-2 hover:text-brand-coral">Pass</button>
          </div>
        </div>
...
function EvidenceRow({ glyph, title, meta }: { glyph: GlyphKey; title: string; meta: string }) {
  return (
    <div className="flex items-center gap-3 py-3 border-b border-brand-near-black/8 last:border-0">
      <GlyphTile glyph={glyph} tone="green" size="md" />
      <div className="flex-1 min-w-0">
        <div className="text-[13.5px] font-semibold text-brand-near-black truncate">
          {title}
        </div>
...
function InfoTile({
  ...
  return (
    <div className={`rounded-xl bg-brand-cream-muted px-3.5 py-3 ${wide ? 'col-span-2' : ''}`}>
