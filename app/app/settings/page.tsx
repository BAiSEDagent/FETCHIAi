...
export default async function SettingsHomePage() {
  ...
  const [profile, sub, signalPrefs] = await Promise.all([
    db.query.serviceProfiles.findFirst({
      where: (t, { eq }) => eq(t.workspaceId, ctx.workspaceId),
    }),
    db.query.workspaceSubscriptions.findFirst({
      where: (t, { eq }) => eq(t.workspaceId, ctx.workspaceId),
    }),
    db.query.signalPreferences.findFirst({
      where: (t, { eq }) => eq(t.workspaceId, ctx.workspaceId),
    }),
  ])
  ...
  const locationLine = profile?.locationCity
    ? `${profile.locationCity}${profile.locationState ? `, ${profile.locationState}` : ''} · ${profile.locationRadiusMiles ?? 50}-mi radius`
    : 'Set your service area'

  const sensitivity =
    (signalPrefs?.minScoreThreshold ?? 70) >= 85
      ? 'Conservative'
      : (signalPrefs?.minScoreThreshold ?? 70) <= 60
        ? 'Aggressive'
        : 'Balanced'

  const howFetchiWorks: Row[] = [
    {
      href: '/app/settings/signals',
      icon: Sparkles,
      iconTone: 'green',
      label: 'Signal sensitivity',
      hint: 'Which signals trigger a lead',
      value: sensitivity,
    },
    {
    ...
    },
    {
      ...
      label: 'Territory',
      hint: 'Where you work',
      value: profile?.locationRadiusMiles
        ? `${profile.locationRadiusMiles} mi`
        : 'Set radius',
    },
    {
      href: '/app/settings/profile',
      ...
      iconTone: 'neutral',
      label: 'Customer list',
      hint: 'Existing customers to skip',
      value: '—',
    },
  ]
  ...
  const account: Row[] = [
    {
      ...
      hint: "Who you are and what you sell",
      value: profile?.vertical ?? 'Set up',
    },
    {
      href: '/app/settings/notifications',
      icon: Bell,
      ...
      <div className="px-4 lg:px-7 pb-10 space-y-5 lg:space-y-6">
        ...
        <div className="rounded-2xl bg-brand-cream shadow-fetchi-soft p-4 lg:p-5">
          <div className="flex items-center gap-3.5">
            <div
              className="w-12 h-12 rounded-full bg-brand-near-black text-white text-[15px] font-bold flex items-center justify-center flex-shrink-0"
              aria-hidden
            >
              {initials}
            </div>
...
function SettingsCardRow({ row, divider }: { row: Row; divider: boolean }) {
  ...
  return (
    <Link
      href={row.href}
      className={`flex items-center gap-3.5 px-4 lg:px-5 py-3.5 min-h-[68px] hover:bg-brand-cream-muted/60 transition-colors ${
        divider ? 'border-b border-brand-near-black/6' : ''
      }`}
    >
      <div
        className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${toneClass}`}
        aria-hidden
      >
