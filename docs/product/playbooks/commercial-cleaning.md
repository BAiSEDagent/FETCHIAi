...
| `exploratory` | Signal is real and tied to a commercial prospect, but the cleaning need is plausible rather than confirmed — e.g. event/venue expansion, ambiguous hiring, or new-business context without a direct cleaning trigger. Use for analyst mode, Other vertical learning, or Aggressive sensitivity review; never for hard disqualifications. | Dimmed chip, "Exploratory — cleaning need plausible but not confirmed" |
