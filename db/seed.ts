...
async function seed() {
  ...
  await db.insert(workspaceSubscriptions).values({
    ...
    trialEndsAt:             new Date(Date.now() + 4 * 24 * 60 * 60 * 1000), // 4 days left
    topupRateCents:          80,
  }).onConflictDoNothing()
...
[{
  ...
  const templates = [
    ...
    {
      slug: 'trial_gate_followup', name: 'Trial Gate Follow-up (24h)',
      subject: 'Your 5 remaining Fetchi leads are waiting',
      bodyHtml: '<p>Hi {{first_name}},</p><p>You saw 5 leads in your trial — the next 5 are ready whenever you are. No charge until your trial ends on {{trial_end_date}}.</p><p><a href="{{app_url}}/app">See my remaining leads →</a></p>',
      bodyText: 'Hi {{first_name}},\n\nYou saw 5 leads in your trial — the next 5 are ready whenever you are. No charge until your trial ends on {{trial_end_date}}.\n\nSee my remaining leads: {{app_url}}/app',
      variables: ['first_name', 'trial_end_date', 'app_url'],
    },
    {
      slug: 'trial_expired', name: 'Trial Expired',
      subject: 'Your Fetchi trial has ended',
      bodyHtml: '<p>Hi {{first_name}},</p><p>Your 7-day trial wrapped up. You found {{leads_found}} leads with Fetchi.</p><p>Pick a plan to keep going: <a href="{{app_url}}/app/expired">Choose a plan →</a></p>',
      bodyText: 'Hi {{first_name}},\n\nYour 7-day trial wrapped up. You found {{leads_found}} leads with Fetchi.\n\nPick a plan to keep going: {{app_url}}/app/expired',
      variables: ['first_name', 'leads_found', 'app_url'],
    },
    {
      slug: 'trial_auto_upgraded', name: 'Trial Auto-Upgraded',
      subject: "You're now on Fetchi Starter",
      bodyHtml: '<p>Hi {{first_name}},</p><p>Your trial ended and your card on file was charged ${{amount}} for the Starter plan. You now have 40 delivered lead cards per month.</p><p><a href="{{app_url}}/app/settings/billing">Manage billing →</a></p>',
      bodyText: 'Hi {{first_name}},\n\nYour trial ended and your card on file was charged ${{amount}} for the Starter plan. You now have 40 delivered lead cards per month.\n\nManage billing: {{app_url}}/app/settings/billing',
  ...
  const agentRegistryData = [
    ...
    },
    {
      ...
      description: 'Sends 24h/48h/7d activation emails. Triggered by trial gate dismissal and onboarding events.',
